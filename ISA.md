...
updated: 2026-09-08T13:50:00+02:00
...
- [x] ISC-10 : La navigation principale reproduit les entrées, l'ordre et les liens du site actuel ; le pied de page, lui, est réorganisé depuis le 2026-09-08 (décision de Michael après la revue de l'accueil) et vit en code dans `src/lib/nav.ts` — `migration/nav.json` n'est plus lu, il reste la trace du crawl. *(Reformulé le 2026-09-08 : la copie conforme du pied de page était un critère de migration, dépassé par l'évolution éditoriale.)*
...
- [ ] ISC-24 : Le rebuild est déclenchable par le webhook de déploiement Hatchbox de l'app (`https://app.hatchbox.io/webhooks/deployments/<token>?latest=true`, donné à Claudy dans `WEBSITE_REBUILD_WEBHOOK_URL`) ; procédure documentée dans `docs/CLAUDY.md` et le README. `[DEFERRED-VERIFY]` jusqu'au premier déploiement réel sur `new.les4sources.be` : une publication dans Claudy doit déclencher un déploiement Hatchbox.
...
- 2026-09-08 09:00 : **Hébergement tranché par Michael : Hatchbox**, sur le serveur de Claudy, app statique `les4sources-website` (créée par lui) avec règles Caddy personnalisées. Faits relevés en lecture seule : Hatchbox a déjà créé la route Caddy `new.les4sources.be` → `/home/deploy/les4sources-website/current/public` (file_server, compression, cache long sur `/assets/*`), le DNS est chez Cloudflare en mode proxy (`new`, `www`, apex et `app` sur les mêmes IP), `new.les4sources.be` répond 404 faute de contenu, bun est absent du serveur (node 24.13 via asdf, `.tool-versions`), le site Semisto y est servi comme dossier statique. Hatchbox détecte Astro (`npm run build` puis `mv public public-original && mv dist public`) et exécute les scripts trouvés dans `.hatchbox/` — nom exact non documenté, d'où un `.hatchbox/build` robuste dans les deux cas (bun installé dans `~/.bun`, lockfile figé, `public/` = site construit, `dist/` laissé en place). Les redirections et normalisations passent par un `deploy/Caddyfile` généré depuis `public/_redirects` (`scripts/redirects-to-caddy.ts`, garde-fou `caddy:check` dans `verify`), à coller dans les réglages de l'app ; le Dockerfile reste comme alternative portable.
- 2026-09-08 13:30 : **Premier déploiement Hatchbox réussi sur `new.les4sources.be`** via l'API Hatchbox (jeton `HATCHBOX_API_TOKEN` fourni par Michael) : app 14061 réglée par `PATCH /apps/14061` (branche `build/astro-v1`, `build_script: bash .hatchbox/build`, `caddyfile`), variables `SITE` et `CLAUDY_PUBLIC_API_URL` par `POST /apps/14061/env_vars`, déploiement automatique par `POST /apps/14061/auto_deploy`, déploiement par `POST /apps/14061/deploy` (journaux `GET /logs/:id`, enfant par serveur). Incident corrigé : Hatchbox remplace la chaîne `%{default}` partout, commentaires compris — mon en-tête la citait, son bloc par défaut a été injecté deux fois (`matcher is defined more than once: @assets`), la validation Caddy a échoué et la sortie du mode maintenance est restée bloquée sur la page 503 ; remise à vide du Caddyfile (route par défaut restaurée), générateur corrigé (marqueur unique en dernière ligne, plus de bloc `handle_errors` : celui d'Hatchbox sert déjà `404.html`), réenregistrement → job Caddy `Valid configuration … Complete!`.

- 2026-09-08 20:50 : **Revue de l'accueil et décisions de Michael, livrées sur `main` (`0a973f8`)** : bouton d'en-tête « Réserver » → `app.les4sources.be/reservation`, tiroir mobile avec Contact et téléphone réservations, événements datés des deux prochains mois (16 fiches migrées datées depuis `properties.Date`), « COMPLET ! » retiré des titres de carte, Guinguette et grappe de 19 liens retirées de l'accueil, GIF de 2 Mo remplacé par la photo de la fiche Pizza Party, soutiens en 2 logos de 120 px, défilement horizontal `scroll-snap` sous 640 px, footer avec adresse complète, téléphone réservations étiqueté, « sous licence » lié, navigation étendue en code (`nav.ts`), `<title>` nettoyés et situés sur 194 pages (`documentTitle()`). Déployé et vérifié en direct (DOM desktop + iPhone 14). Détail : `~/.claude/PAI/MEMORY/WORK/les4sources-home-review/ISA.md`.
- 2026-09-08 23:05 : **Chantier « pages internes »** ouvert sur les retours de Michael : photos floues (la migration n'avait que la variante `/public` du CDN de Super, ~1 150 px ; le même CDN sert `w=2400` avec un User-Agent de navigateur → `scripts/upgrade-images.ts`), page Salles à restructurer, photo de la cuisine à refaire (mémoire projet), contenu de chaque page allégé sans changer le ton, grappe de « Séjours » transformée en menu, bande réservation rendue par le gabarit. Détail : `~/.claude/PAI/MEMORY/WORK/les4sources-pages-internes/ISA.md`.
- 2026-09-09 15:00 : **Fiche événement refaite, pictos de pôles colorés au survol, plus d'emoji de tête de page (`a5465e1`)**. `PageHero` n'affiche plus `icon` ; le pied de page allume chaque picto dans la couleur de son pôle au survol (`.l4s-pole`, global.css) ; la fiche événement gagne `EventHero` (photo + aplat du pôle, empilés sur mobile), `EventPractical` (carte collante : date, horaire, lieu, tarif, type, bouton avec le libellé de l'éditrice, `.ics`, partage, autres dates de la série), un fichier iCalendar par événement (`src/pages/evenements/[slug].ics.ts`, `src/lib/ics.ts`), les états passé et complet, une barre collante mobile, le menu « Pratique pour ta venue » ; JSON-LD enrichi (prix, disponibilité, image). Nouveau champ `registrationLabel` (schéma `evenements`, libellé migré sans emoji). 71 fiches migrées allégées par script (boutons, listes de relations, titres en gras, alt). Détail : `~/.claude/PAI/MEMORY/WORK/les4sources-fiche-evenement/ISA.md`.
...
- conjectured : la copie conforme (structure, navigation, textes au mot près) était l'état final du site.
  refuted_by : revue de l'accueil du 2026-09-08 — Michael a tranché 16 évolutions éditoriales (CTA, footer, sections retirées, titres), puis demandé une passe de lisibilité sur toutes les pages.
  learned : la parité n'était que le point de départ ; à partir de maintenant le contenu du dépôt est la source éditoriale et s'éloigne du site Super à mesure des décisions. `scripts/verify/parity.ts` reste un outil de diagnostic, plus un critère.
  criterion_now : ISC-7 et ISC-10 lus comme des critères de migration, clos ; les évolutions vivent dans les ISA de travail.
...
- ISC-24 partiel / ISC-29 sur Caddy (2026-09-08 09:05) : Caddy 2.11.4 local (`brew install caddy`), `caddy validate` → « Valid configuration » sur `deploy/Caddyfile` complété du bloc statique d'Hatchbox (root, encode, file_server) ; sondes curl contre `dist/` : `/` 200 ; `/agenda` 200 sans redirection ; `/agenda/` 301 → `/agenda` ; `/sejours/hebergements-yvoir/la-hulotte-gite-16-personnes` 200 ; `/catalogue/atelier-fabrication-de-pizza` 301 → `/catalogue` ; `/teams-buildings` 301 → `/un-team-building-aux-4-sources` ; `/projets/le-bar-des-4-sources` 301 → `/le-bar-des-4-sources` ; `/_redirects` 404 ; `/page-inexistante` 404 avec `<title>Page introuvable</title>` ; `Host: les4sources.be` `/sejours?x=1` 301 → `https://www.les4sources.be/sejours?x=1` ; `Host: www.les4sources.be` 200 ; `/_astro/*.css` `Cache-Control: public, max-age=31536000, immutable` ; `Content-Encoding: gzip`. Reste à prouver sur `new.les4sources.be` après le premier déploiement Hatchbox (réglages à faire par Michael ou via l'API).
- ISC-24 partiel / ISC-29 / ISC-6 sur le site déployé (2026-09-08 13:35, `https://new.les4sources.be`, Cloudflare devant Hatchbox) : journal de déploiement 4231626 puis 4231663 → `.hatchbox/build` exécuté (`bun 1.4.2`, `429 packages installed`, `SITE=https://www.les4sources.be`, `CLAUDY_PUBLIC_API_URL=https://app.les4sources.be/api/public/v1`, `195 page(s) built`, `✓ deploy/Caddyfile à jour`, `site construit : 194 pages dans public/`, `Deploy complete`) ; `claudy: source=api events=0 experiences=0 merged=0 legacy_only=115` (l'API n'est pas encore déployée côté Claudy, repli sans casse) ; job Caddy 4231699 → « Valid configuration », chargé sur `localhost:2019/load` ; sondes : `/agenda` 200 sans redirection, `/agenda/` 301 → `/agenda`, `/teams-buildings` 301 → `/un-team-building-aux-4-sources`, `/_redirects` 404, `/page-inexistante` 404 avec `<title>Page introuvable</title>`, `/_astro/*.css` `cache-control: public, max-age=31536000, immutable`, canonical de `/agenda` = `https://www.les4sources.be/agenda` ; **209/209 URLs de `migration/urls.txt` répondent 200 ou 301** sur le site déployé. Reste pour clore ISC-24 : `WEBSITE_REBUILD_WEBHOOK_URL` (webhook « Trigger a deploy » de l'app, à copier depuis le tableau de bord) posé sur l'app Claudy après déploiement de la PR #259, puis une publication dans Claudy qui déclenche un déploiement.
- ISC-21/22/24 partiel contre Claudy réel (2026-09-08 13:45) : PR #1 mergée (`d484e08`), app Hatchbox passée sur `main` (`PATCH /apps/14061 {branch: main}`), déploiement 4231740 terminé (`Deploy complete`, 194 pages) ; l'API publique de production répond (`https://app.les4sources.be/api/public/v1/*` → 200, cache 300 s, ETag), le build la lit (variable `CLAUDY_PUBLIC_API_URL` de l'app) ; rien n'est encore publié côté Claudy, donc `events=0 experiences=0` et repli complet sur le contenu migré. `WEBSITE_REBUILD_WEBHOOK_URL` posée sur Claudy par Michael. ISC-24 reste `[DEFERRED-VERIFY]` jusqu'à la première publication observée (déploiement déclenché par le webhook).
